...
$(document).ready(function () {
  ...
  function sendFormData(data) {
    ...
    return fetch("ajax/createResult.php", {
      ...
      .then((res) => res.json())
      .then((data) => {
        question_answers.style.display = "block";
    ...
    let newChart = `
        <div class="row" id="resultado">
            ...
            <div class="col graficoText mt-3">
                <div>
                    <h1>Hay <span>${
                      data["response"]["sameOpinion"]
                    }</span> personas que opinan lo mismo que tú</h1>
                    <img src="assets/img/gif/gif_N(${randomGIF}).gif" class="img-fluid mb-3" alt="Gif">
                    ${
                      data["response"]["message"] != ""
                        ? `<p>Tu match te dice "<span>${data["response"]["message"]}</span>"</p>`
                        : ""
                    }
                </div>
            </div>
  ...
  function characterRestriction() {
    ...
    textarea.addEventListener("input", function () {
      if (textarea.value.length > maxLength) {
        ...
        alertDiv(
          "Error número de caracteres",
          "no se puede exeder de los 255 carácteres.",
  ...
  function messageLoginBtnError() {
    ...
    messageLoginBtn.addEventListener("click", function () {
      ...
      alertDiv(
        ...
        " Debe inicar sesión para poder enviar un comentario o mensaje",
        errorDivLogin
      );
    });
  }
